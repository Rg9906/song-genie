...
Integrates web scraping, human relevance validation, and redundancy management
"""
...
class UltimateDynamicSystem:
    """The ultimate dynamic question generation system"""
    
    def __init__(self, songs: List[Dict[str, Any]], use_web_scraping: bool = False):
        self.songs = songs
    ...
    def _initialize_components(self):
        ...
        try:
            from .dynamic_web_scraper import DynamicWebScraper
            self.web_scraper = DynamicWebScraper()
        ...
        try:
            ...
            logger.info("🧠 Human relevance validator initialized")
        except ImportError:
    ...
    def _discover_dynamic_attributes(self) -> Dict[str, Dict]:
        """Discover attributes from songs and potentially scrape more"""
        attributes = {}
        
        # Discover from existing songs
        ...
        # Optionally scrape additional attributes
        if self.use_web_scraping and self.web_scraper:
            logger.info("🌐 Scraping additional song information...")
            try:
                # Sample a few songs to scrape
                sample_songs = random.sample(self.songs, min(5, len(self.songs)))
                scraped_songs = self.web_scraper.batch_scrape_songs(sample_songs)
                
                # Merge scraped attributes
                for scraped_song in scraped_songs:
                    for attr, value in scraped_song.items():
                        if attr.startswith('scraped_'):
                            clean_attr = attr.replace('scraped_', '')
                            ...
                            attr_info = attributes[clean_attr]
                            attr_info['source'] = 'scraped'
                ...
                logger.info(f"✅ Scraped additional attributes for {len(sample_songs)} songs")
    ...
    def _load_dynamic_patterns(self) -> Dict[str, List[str]]:
        """Load dynamic question patterns based on discovered attributes"""
        patterns = {
            'human_friendly': [
    ...
    def suggest_improvements(self) -> List[str]:
        ...
        total_attrs = len(self.dynamic_attributes)
        if total_attrs < 10:
            suggestions.append("Consider adding more diverse song attributes")
        
        # Check web scraping
        scraped_attrs = len([a for a, info in self.dynamic_attributes.items() if info.get('source') == 'scraped'])
        if scraped_attrs == 0 and self.use_web_scraping:
            suggestions.append("Web scraping is enabled but no scraped attributes found")
        
        # Check redundancy management
        if self.redundancy_manager:
            redundancy_suggestions = self.redundancy_manager.suggest_diversity_improvements()
